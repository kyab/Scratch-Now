...
if [ -n "${FFMPEG_BIN}" ]; then
  ...
  if [ -n "${SCREEN_IDX}" ]; then
    ...
    # Capture as rawvideo (zero-CPU): encoding with x264 while the test runs
    # competes with the runner's fragile virtual audio stack and was correlated
    # with periodic clicks in the captured audio. The raw capture is a few GB
    # but is re-encoded offline (and deleted) right after the test. Keep the
    # native display resolution and frame timing (no upscale / interpolation).
