...
5. 証拠動画 `evidence.mp4` を CI アーティファクトに保存する:
   - ffmpeg（avfoundation, `-capture_cursor 1`）でマウスポインタ込みの画面を 60 fps で録画
   - **録画中は rawvideo（無エンコード）で書き出す**: テスト実行中の x264
     リアルタイムエンコードは CPU 負荷でランナーの仮想オーディオを乱し、
     キャプチャ音声に周期的なクリックノイズを生む。生キャプチャは数 GB になるが
     mux 後に削除する。x264 エンコード・2 倍拡大（lanczos）は
     テスト終了後に `make_evidence_video.py` がオフラインで行う
     （仮想ディスプレイが 1024x768 固定のため、拡大はアップスケール）
   - 仮想ディスプレイのフレーム供給は不規則な実測 ~27 fps（間隔 15〜100 ms）
     でジャダーが目立つため、mux 時に動き補間（`minterpolate` mci）で
     滑らかな 60 fps CFR に変換する（回転マーカーに軽い残像が出るのは
     ブロックベース補間の仕様）
   - 証拠動画の音声トラックには `adeclick` を適用してランナー由来の
     クリックノイズを除去する（**テストのアサーションは無加工の信号**で実施。
     フィルタ前後のクリック数は mux ログに出る）
   - 同様の理由で、アプリ側 CI フック（JSONL / PCM 書き込み）はオーディオ
     スレッドから一切ディスク I/O をせず、シリアルなバックグラウンドキューへ
     ディスパッチする（レンダー期限超過によるキャプチャ欠落 = クリック源）
   - **CI ビルドは大きな IO バッファで動かす**: 製品の低レイテンシ設定
     （キャプチャ 64 / 出力 32 フレーム ≈ 1.3 / 0.7 ms 周期）は VM では
     締め切りを守れないため、`SCRATCH_NOW_TAP_SMOKE_CI` ビルドのみ
     キャプチャ 4096 / 出力 1024 フレーム（≈ 85 / 21 ms 周期）に緩和し、
     RingBuffer のライブ追従マージンも 8192 フレームに拡大する
     （レイテンシ ~170 ms はテストの合否に影響しない。出力側は
     AppController の可変レート一時バッファが 1024 フレーム固定のため上限 1024）
   ...
   - **音声クロック校正**: ランナーの仮想オーディオデバイスは公称 48 kHz より
     2〜3% 遅く消費するため、48 kHz 前提で mux すると 35 秒で 1 秒以上ズレる。
     `output.jsonl` の「壁時計 vs 累積レンダフレーム数」を最小二乗フィットして
     実効サンプルレートと PCM 先頭サンプルの壁時計時刻を求め、raw PCM を
     実効レートとして解釈させることでドリフトを除去する
   - 映像側は「録画停止時刻 − 最終ビデオパケット pts」で開始時刻を逆算（±0.1 秒程度）
   - mux 後に音声トラックの無音区間（tableStop）のエッジを `phases.json` の
     クリック時刻と突き合わせ、残留ズレをログに出す（自己診断）
   - 動画左上に経過時間を焼き込み、目視でも同期を確認できるようにする
