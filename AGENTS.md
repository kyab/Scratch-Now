...
### モデルの選択
ユーザーが選択したモデルから上限到達などで違うモデルにする場合は、勝手にモデル変更して続行しないで、
切り替えて続行するか、キャンセルするかユーザーに確認すること。

### 開発者の想定
- ユーザーがDSPの専門家と想定しないこと。DSP、音響における専門用語については適時解説を入れること。
...
途中でのへんこうは「割り込み」用語を使うことにする。
例：停止ボタンで停止にむけて減速中に２本指スクロールが割り込む
