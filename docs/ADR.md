...
# Scrollイベントによる２本指スクラッチ（廃止）
`scrollWheel:`イベントハンドラを使った仮実装までした。が、スクロール量の量子化が粗いため２本指スクラッチとしては NSTouchを使うことにした。
なおScrollイベントは惰性分もイベントが発火するので、精度が粗くても惰性付きイベントが欲しい場合の選択肢にはなりうる。
